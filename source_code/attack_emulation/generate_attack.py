...
def generate_attack_train(benign_data, attack_begin_date, attack_end_date, attacked_ratio_nodes, attack_duration,
    ...
    output_path +=  "attacked_data_" + data_type + '_' + str(attack_begin_date).replace(':', '_').replace(' ', '_') + '_' + str(attack_end_date).replace(':', '_').replace(' ', '_') +\
                    "_ratio_" + str(attacked_ratio_nodes) + "_duration_" + str(attack_duration).replace(':', '_').replace(' ', '_') + ".csv"
...
def main_generate_attack(benign_dataset_path, data_type, num_train_days, num_test_days):
    ...
    if data_type == "train":
        ...
        output_path = CONFIG.OUTPUT_DIRECTORY + "\\attack_emulation\\Output\\attacked_data\\train\\"
    else:
        ...
        output_path = CONFIG.OUTPUT_DIRECTORY + "\\attack_emulation\\Output\\attacked_data\\test\\"
    ...
    output_path_benign = output_path +  "attacked_data_" + data_type + '_' + str(attack_begin_date).replace(':', '_').replace(' ', '_') + '_' +\
                         str(attack_end_date).replace(':', '_').replace(' ', '_') + "_ratio_0_duration_0.csv"
