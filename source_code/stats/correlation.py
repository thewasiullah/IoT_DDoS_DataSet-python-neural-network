...
def load_dataset(path):
    ...
    #data["NODE"] = pd.to_numeric(data["NODE"], errors = 'ignore')
    return data
...
def generate_correlation_data(data1, data2, plot_data, plot_data_rows):
    ...
    print(data1["LNG"][0])
    print(data2["LNG"][0])
    dist = math.sqrt((float(data1["LAT"][0]) - float(data2["LAT"][0])) ** 2 + float((data1["LNG"][0]) - float(data2["LNG"][0])) ** 2)
...
def main_generate_correlation_data():
    ...
    benign_data["NODE"] = benign_data["NODE"].astype(int, errors="ignore")
