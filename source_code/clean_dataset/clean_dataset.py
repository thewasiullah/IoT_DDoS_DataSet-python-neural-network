...
def sort_original_dataset_by_date():
    print("Sorting original dataset by datetime")
    data = pd.read_csv(CONFIG.ORIGINAL_DATASET_PATH)
    data["TIME"] = pd.to_datetime(data["TIME"])
    sorteddata = data.sort_values(by=["TIME"])
    #data = pd.DataFrame(columns=["NODE", "LAT", "LNG", "TIME", "ACTIVE"])
    sorteddata.to_csv(os.path.join(CONFIG.DATASET_PATH + '\dataset', 'original_data_sorted.csv'), index=False)
